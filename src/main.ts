...
export class ProcessBar {
  ...
  private line = 0;
  private lineStep = 0;
  private cursorStart = 30;
  private startTime: number = 0;
  private etaSmoothed: number | null = null; // Persistent across calls
  constructor(
  ...
  next() {
    ...
    if (dif >= 1) {
      dif = Math.round(dif);
      this.line += dif;
      if (this.line > this.lineLength) this.line = this.lineLength;
      this.plotBar();
    }
  ...
  private cursorToEndBar() {
    let end: number = this.lineLength + this.cursorStart + 1;
    process.stdout.cursorTo(end);
  }

  private cursorToEndText(text:string) {
    this.cursorToEndBar()
  ...
  private plotRate(success: number, eta: string) {
